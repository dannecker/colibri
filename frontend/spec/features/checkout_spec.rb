...
describe "Checkout", inaccessible: true do
  ...
  context "visitor makes checkout as guest without registration" do
    ...
    context "defaults to use billing address" do
      before do
        add_mug_to_cart
        Colibri::Order.last.update_column(:email, "test@example.com")
    ...
    context "persists state when on address page" do
      ...
      specify do
        Colibri::Order.count.should == 1
        Colibri::Order.last.state.should == "address"
      end
    end
  ...
  context "when Colibri::Config[:always_include_confirm_step] is true" do
    before do
  ...
  context "and likes to double click buttons" do
    ...
    it "prevents double clicking the payment button on checkout", :js => true do
      visit colibri.checkout_state_path(:payment)
    ...
    it "prevents double clicking the confirm button on checkout", :js => true do
      ...
      visit colibri.checkout_state_path(:confirm)
  ...
  context "when several payment methods are available" do
    ...
    before do
      ...
      Colibri::CheckoutController.any_instance.stub(current_order: order)
      Colibri::CheckoutController.any_instance.stub(try_colibri_current_user: order.user)
  ...
  context "user has payment sources", js: true do
    ...
    it "selects first source available and customer moves on" do
      ...
      }.not_to change { Colibri::CreditCard.count }

      click_on "Place Order"
      expect(current_path).to eql(colibri.order_path(Colibri::Order.last))
    end

    it "allows user to enter a new source" do
      choose "use_existing_card_no"

      fill_in "Name on card", :with => 'Colibri Commerce'
      ...
      expect {
        click_on "Save and Continue"
      }.to change { Colibri::CreditCard.count }.by 1
  ...
  context "goes back from payment to add another item", js: true do
    ...
    it "transit nicely through checkout steps again" do
      ...
      click_on "Save and Continue"
      click_on "Save and Continue"
      expect(current_path).to eql(colibri.checkout_state_path("payment"))
  ...
  context "from payment step customer goes back to cart", js: true do
    ...
    context "and updates line item quantity and try to reach payment page" do
      ...
      it "redirects user back to address step" do
        visit colibri.checkout_state_path("payment")
        expect(current_path).to eql(colibri.checkout_state_path("address"))
      end
      ...
      it "updates shipments properly through step address -> delivery transitions" do
        ...
        expect(Colibri::InventoryUnit.count).to eq 3
      end
    end
    ...
    context "and adds new product to cart and try to reach payment page" do
      ...
      it "updates shipments properly through step address -> delivery transitions" do
        ...
        expect(Colibri::InventoryUnit.count).to eq 2
      end
    end
  end

  context "in coupon promotion, submits coupon along with payment", js: true do
    let!(:promotion) { Colibri::Promotion.create(name: "Huhuhu", code: "huhu") }
    let!(:calculator) { Colibri::Calculator::FlatPercentItemTotal.create(preferred_flat_percent: "10") }
    let!(:action) { Colibri::Promotion::Actions::CreateItemAdjustments.create(calculator: calculator) }
    ...
    it "makes sure payment reflects order total with discounts" do
      ...
      expect(Colibri::Payment.first.amount.to_f).to eq Colibri::Order.last.total.to_f
    end
    ...
    context "invalid coupon" do
      it "doesnt create a payment record" do
        ...
        expect(Colibri::Payment.count).to eq 0
        expect(page).to have_content(Colibri.t(:coupon_code_not_found))
      end
    end
  ...
  context "order has only payment step" do
    before do
      ...
      @old_checkout_flow = Colibri::Order.checkout_flow
      ...
      Colibri::Order.any_instance.stub email: "colibri@commerce.com"
    ...
    after do
      Colibri::Order.checkout_flow(&@old_checkout_flow)
    end

    it "goes right payment step and place order just fine" do
      expect(current_path).to eq colibri.checkout_state_path('payment')
      ...
      expect(current_path).to eq colibri.checkout_state_path('confirm')
      click_button "Place Order"
    end
  end
  ...
  context "save my address" do
    ...
    context 'as a User' do
      before do
        Colibri::CheckoutController.any_instance.stub(:try_colibri_current_user => create(:user))
        click_button "Checkout"
      end
  ...
  context "when order is completed" do
    ...
    before(:each) do
      Colibri::CheckoutController.any_instance.stub(:current_order => order)
      Colibri::CheckoutController.any_instance.stub(:try_colibri_current_user => user)
      Colibri::OrdersController.any_instance.stub(:try_colibri_current_user => user)
    ...
    it "displays a thank you message" do
      expect(page).to have_content(Colibri.t(:thank_you_for_your_order))
    end

    it "does not display a thank you message on that order future visits" do
      visit colibri.order_path(order)
      expect(page).to_not have_content(Colibri.t(:thank_you_for_your_order))
    end
  end
