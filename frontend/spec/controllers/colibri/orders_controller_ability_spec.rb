...
module Colibri
  describe OrdersController do
    ...
    it 'should understand order routes with token' do
      colibri.token_order_path('R123456', 'ABCDEF').should == '/orders/R123456/token/ABCDEF'
    end

    context 'when no order exists in the session' do
      before { Colibri::Order.stub :new => order }
    ...
    context 'when an order exists in the session' do
      ...
      context '#edit' do
        it 'should check if user is authorized for :edit' do
          controller.should_receive(:authorize!).with(:edit, order, token)
          colibri_get :edit, :token => token
        end
        it "should check against the specified order" do
          controller.should_receive(:authorize!).with(:edit, specified_order, token)
          colibri_get :edit, :id => specified_order.number, :token => token
        end
      end
      ...
      context '#update' do
        it 'should check if user is authorized for :edit' do
          ...
          controller.should_receive(:authorize!).with(:edit, order, token)
          colibri_post :update, :order => { :email => "foo@bar.com" }, :token => token
        end
        it "should check against the specified order" do
          order.stub :update_attributes
          controller.should_receive(:authorize!).with(:edit, specified_order, token)
          colibri_post :update, :order => { :email => "foo@bar.com" }, :id => specified_order.number, :token => token
        end
      end
      ...
      context '#empty' do
        it 'should check if user is authorized for :edit' do
          controller.should_receive(:authorize!).with(:edit, order, token)
          colibri_post :empty, :token => token
        end
        it "should check against the specified order" do
          controller.should_receive(:authorize!).with(:edit, specified_order, token)
          colibri_post :empty, :id => specified_order.number, :token => token
        end
      end
      ...
      context "#show" do
        it "should check against the specified order" do
          controller.should_receive(:authorize!).with(:edit, specified_order, token)
          colibri_get :show, :id => specified_order.number, :token => token
        end
      end
    end
      ...
      context '#show' do
        context 'when token parameter present' do
          it 'should store as guest_token in session' do
            colibri_get :show, {:id => 'R123', :token => order.token }
