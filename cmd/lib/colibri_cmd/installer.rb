...
module ColibriCmd
  ...
  class Installer < Thor::Group
    ...
    desc 'Creates a new rails project with a colibri store'
    ...
    class_option :version, :type => :string, :desc => 'Colibri Version to use'

    class_option :edge, :type => :boolean

    class_option :path, :type => :string, :desc => 'Colibri gem path'
    class_option :git, :type => :string, :desc => 'Colibri gem git url'
    class_option :ref, :type => :string, :desc => 'Colibri gem git ref'
    class_option :branch, :type => :string, :desc => 'Colibri gem git branch'
    class_option :tag, :type => :string, :desc => 'Colibri gem git tag'
    ...
    def prepare_options
      ...
      if options[:edge]
        @colibri_gem_options[:git] = 'https://github.com/colibri/colibri.git'
      elsif options[:path]
        @colibri_gem_options[:path] = options[:path]
      elsif options[:git]
        @colibri_gem_options[:git] = options[:git]
        @colibri_gem_options[:ref] = options[:ref] if options[:ref]
        @colibri_gem_options[:branch] = options[:branch] if options[:branch]
        @colibri_gem_options[:tag] = options[:tag] if options[:tag]
      elsif options[:version]
        @colibri_gem_options[:version] = options[:version]
      else
        version = Gem.loaded_specs['colibri_cmd'].version
        @colibri_gem_options[:version] = version.to_s
      end
    end
    ...
    def add_gems
      inside @app_path do
        ...
        gem :colibri, @colibri_gem_options

        if @install_default_gateways
          gem :colibri_gateway, :git => "https://github.com/colibri/colibri_gateway.git", :branch => "2-1-stable"
        end

        if @install_default_auth
          gem :colibri_auth_devise, :git => "https://github.com/colibri/colibri_auth_devise.git", :branch => "2-1-stable"
        end
    ...
    def initialize_colibri
      colibri_options = []
      colibri_options << "--migrate=#{@run_migrations}"
      colibri_options << "--seed=#{@load_seed_data}"
      colibri_options << "--sample=#{@load_sample_data}"
      colibri_options << "--user_class=#{@user_class}"
      colibri_options << "--auto_accept" if options[:auto_accept]

      inside @app_path do
        run "rails generate colibri:install #{colibri_options.join(' ')}", :verbose => false
      end
    end
