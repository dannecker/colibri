module Colibri
  module BaseHelper
    ...
    # Defined because Rails' current_page? helper is not working when Colibri is mounted at root.
    def current_colibri_page?(url)
      path = request.fullpath.gsub(/^\/\//, '/')
      if url.is_a?(String)
        return path == url
      elsif url.is_a?(Hash)
        return path == colibri.url_for(url)
      end
      return false
    end

    def link_to_cart(text = nil)
      text = text ? h(text) : Colibri.t('cart')
      css_class = nil

      if simple_current_order.nil? or simple_current_order.item_count.zero?
        text = "#{text}: (#{Colibri.t('empty')})"
        css_class = 'empty'
      else
      ...
      link_to text, colibri.cart_path, :class => "cart-info #{css_class}"
    end
    ...
    def meta_data
      ...
      if meta[:description].blank? && object.kind_of?(Colibri::Product)
    ...
    def logo(image_path=Colibri::Config[:logo])
      link_to image_tag(image_path), colibri.root_path
    end
    ...
    def breadcrumbs(taxon, separator="&nbsp;&raquo;&nbsp;")
      ...
      crumbs = [content_tag(:li, link_to(Colibri.t(:home), colibri.root_path) + separator)]
      if taxon
        crumbs << content_tag(:li, link_to(Colibri.t(:products), products_path) + separator)
        crumbs << taxon.ancestors.collect { |ancestor| content_tag(:li, link_to(ancestor.name , seo_url(ancestor)) + separator) } unless taxon.ancestors.empty?
        crumbs << content_tag(:li, content_tag(:span, link_to(taxon.name , seo_url(taxon))))
      else
        crumbs << content_tag(:li, content_tag(:span, Colibri.t(:products)))
      end
    ...
    def available_countries
      checkout_zone = Zone.find_by(name: Colibri::Config[:checkout_zone])
      ...
      countries.collect do |country|
        country.name = Colibri.t(country.iso, scope: 'country_names', default: country.name)
        country
      end.sort { |a, b| a.name.parameterize <=> b.name.parameterize }
    end

    def seo_url(taxon)
      return colibri.nested_taxons_path(taxon.permalink)
    end
    ...
    def image_style_from_method_name(method_name)
      if style = method_name.to_s.sub(/_image$/, '')
        possible_styles = Colibri::Image.attachment_definitions[:attachment][:styles]
    ...
    def define_image_method(style)
      self.class.send :define_method, "#{style}_image" do |product, *options|
        ...
        if product.images.empty?
          if !product.is_a?(Colibri::Variant) && !product.variant_images.empty?
