      ...
      class CreateAdjustment < PromotionAction
        ...
        def perform(options = {})
          ...
          Colibri::Adjustment.create!(
            ...
            adjustable: order,
            source: self,
            label: "#{Colibri.t(:promotion)} (#{promotion.name})"
          )
          true
        end
        ...
        private
          ...
          def deals_with_adjustments
            adjustment_scope = self.adjustments.joins("LEFT OUTER JOIN colibri_orders ON colibri_orders.id = colibri_adjustments.adjustable_id")
            # For incomplete orders, remove the adjustment completely.
            adjustment_scope.where("colibri_orders.completed_at IS NULL").readonly(false).destroy_all

            # For complete orders, the source will be invalid.
            # Therefore we nullify the source_id, leaving the adjustment in place.
            # This would mean that the order's total is not altered at all.
            adjustment_scope.where("colibri_orders.completed_at IS NOT NULL").update_all("source_id = NULL")
          end
      end
    end
  end
