# Use singleton class Colibri::Preferences::Store.instance to access
#
...
module Colibri::Preferences
  ...
  class StoreInstance
    ...
    def exist?(key)
      @cache.exist?(key) ||
      should_persist? && Colibri::Preference.where(:key => key).exists?
    end
    ...
    def get(key)
      ...
      if should_persist?
        ...
        if preference = Colibri::Preference.find_by_key(key)
          # it does exist
    ...
    def persist(cache_key, value)
      ...
      preference = Colibri::Preference.where(:key => cache_key).first_or_initialize
    ...
    def destroy(cache_key)
      ...
      preference = Colibri::Preference.find_by_key(cache_key)
      preference.destroy if preference
    end

    def should_persist?
      @persistence and Colibri::Preference.table_exists?
    end
