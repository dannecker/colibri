...
module Colibri
  class Adjustment < Colibri::Base
    ...
    scope :tax, -> { where(source_type: 'Colibri::TaxRate') }
    scope :price, -> { where(adjustable_type: 'Colibri::LineItem') }
    scope :shipping, -> { where(adjustable_type: 'Colibri::Shipment') }
    scope :optional, -> { where(mandatory: false) }
    scope :eligible, -> { where(eligible: true) }
    scope :charge, -> { where("#{quoted_table_name}.amount >= 0") }
    scope :credit, -> { where("#{quoted_table_name}.amount < 0") }
    scope :promotion, -> { where(source_type: 'Colibri::PromotionAction') }
    scope :return_authorization, -> { where(source_type: "Colibri::ReturnAuthorization") }
    ...
    def promotion?
      source.class < Colibri::PromotionAction
    end
    ...
    def currency
      adjustable ? adjustable.currency : Colibri::Config[:currency]
    end
    ...
    def update_adjustable_adjustment_total
      ...
      Colibri::ItemAdjustments.new(adjustable).update if adjustable
    end
  end
end
