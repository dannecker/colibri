...
require 'colibri/order/checkout'

module Colibri
  class Order < Colibri::Base
    include Checkout
    ...
    if Colibri.user_class
      ...
      belongs_to :created_by, class_name: Colibri.user_class.to_s
      belongs_to :approver, class_name: Colibri.user_class.to_s
    else
      belongs_to :user
    ...
    belongs_to :bill_address, foreign_key: :bill_address_id, class_name: 'Colibri::Address'
    alias_attribute :billing_address, :bill_address

    belongs_to :ship_address, foreign_key: :ship_address_id, class_name: 'Colibri::Address'
    ...
    has_and_belongs_to_many :promotions, join_table: 'colibri_orders_promotions'
    ...
    def self.by_customer(customer)
      joins(:user).where("#{Colibri.user_class.table_name}.email" => customer)
    end
    ...
    def all_adjustments
      Adjustment.where("order_id = :order_id OR (adjustable_id = :order_id AND adjustable_type = 'Colibri::Order')",
        :order_id => self.id)
    end
    ...
    def currency
      self[:currency] || Colibri::Config[:currency]
    end

    def display_outstanding_balance
      Colibri::Money.new(outstanding_balance, { currency: currency })
    end

    def display_item_total
      Colibri::Money.new(item_total, { currency: currency })
    end

    def display_adjustment_total
      Colibri::Money.new(adjustment_total, { currency: currency })
    end

    def display_included_tax_total
      Colibri::Money.new(included_tax_total, { currency: currency })
    end

    def display_additional_tax_total
      Colibri::Money.new(additional_tax_total, { currency: currency })
    end

    def display_tax_total
      Colibri::Money.new(included_tax_total + additional_tax_total, { currency: currency })
    end

    def display_shipment_total
      Colibri::Money.new(shipment_total, { currency: currency })
    end
    alias :display_ship_total :display_shipment_total

    def display_total
      Colibri::Money.new(total, { currency: currency })
    end
    ...
    def confirmation_required?
      Colibri::Config[:always_include_confirm_step] ||
    ...
    def tax_address
      Colibri::Config[:tax_using_ship_address] ? ship_address : bill_address
    end
    ...
    def contents
      @contents ||= Colibri::OrderContents.new(self)
    end
    ...
    def create_tax_charge!
      ...
      Colibri::TaxRate.adjust(order_tax_zone, line_items)
      Colibri::TaxRate.adjust(order_tax_zone, shipments) if shipments.any?
    end
    ...
    def process_payments!
      if pending_payments.empty?
        raise Core::GatewayError.new Colibri.t(:no_pending_payments)
      else
      ...
      result = !!Colibri::Config[:allow_checkout_on_gateway_error]
    ...
    def can_add_coupon?
      Colibri::Promotion.order_activatable?(self)
    end
    ...
    def create_proposed_shipments
      ...
      packages = Colibri::Stock::Coordinator.new(self).packages
    ...
    def apply_free_shipping_promotions
      Colibri::PromotionHandler::FreeShipping.new(self).activate
    ...
    private
      ...
      def ensure_line_items_present
        unless line_items.present?
          errors.add(:base, Colibri.t(:there_are_no_items_for_this_order)) and return false
        end
      end
      ...
      def ensure_available_shipping_rates
        if shipments.empty? || shipments.any? { |shipment| shipment.shipping_rates.blank? }
          ...
          errors.add(:base, Colibri.t(:items_cannot_be_shipped)) and return false
        end
      end
      ...
      def set_currency
        self.currency = Colibri::Config[:currency] if self[:currency].nil?
      end
