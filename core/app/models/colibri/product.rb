...
module Colibri
  class Product < Colibri::Base
    ...
    has_and_belongs_to_many :promotion_rules, join_table: :colibri_products_promotion_rules

    belongs_to :tax_category, class_name: 'Colibri::TaxCategory'
    belongs_to :shipping_category, class_name: 'Colibri::ShippingCategory', inverse_of: :products

    has_one :master,
      -> { where is_master: true },
      inverse_of: :product,
      class_name: 'Colibri::Variant',
      dependent: :destroy

    has_many :variants,
      -> { where(is_master: false).order("#{::Colibri::Variant.quoted_table_name}.position ASC") },
      inverse_of: :product,
      class_name: 'Colibri::Variant'

    has_many :variants_including_master,
      -> { order("#{::Colibri::Variant.quoted_table_name}.position ASC") },
      inverse_of: :product,
      class_name: 'Colibri::Variant',
      dependent: :destroy

    has_many :prices, -> { order('colibri_variants.position, colibri_variants.id, currency') }, through: :variants
    ...
    validates :price, presence: true, if: proc { Colibri::Config[:require_master_price] }
    ...
    def possible_promotions
      ...
      Colibri::Promotion.advertised.where(id: promotion_ids).reject(&:expired?)
    end
    ...
    private
      ...
      def add_properties_and_option_types_from_prototype
        if prototype_id && prototype = Colibri::Prototype.find_by(id: prototype_id)
...
require_dependency 'colibri/product/scopes'
