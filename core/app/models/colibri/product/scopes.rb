module Colibri
  class Product < Colibri::Base
    ...
    #
    #   Colibri::Product.joins(:taxons).where(Taxon.table_name => { :id => taxon.id })
    #
    # If you're using count on the result of this scope, you must use the
    # `:distinct` option as well:
    #
    #   Colibri::Product.in_taxon(taxon).count(:distinct => true)
    #
    # This is so that the count query is distinct'd:
    #
    #   SELECT COUNT(DISTINCT "colibri_products"."id") ...
    #
    #   vs.
    #
    #   SELECT COUNT(*) ...
    add_search_scope :in_taxon do |taxon|
      ...
      where("colibri_products_taxons.taxon_id" => taxon.self_and_descendants.pluck(:id)).
      order("colibri_products_taxons.position ASC")
    end

    # This scope selects products in all taxons AND all its descendants
    # If you need products only within one taxon use
    #
    #   Colibri::Product.taxons_id_eq([x,y])
    ...
    add_search_scope :with_option_value do |option, value|
      ...
      group('colibri_products.id').joins(variants_including_master: :option_values).where(conditions)
    end
    ...
    add_search_scope :taxons_name_eq do |name|
      group("colibri_products.id").joins(:taxons).where(Taxon.arel_table[:name].eq(name))
    end
    ...
    def self.distinct_by_product_ids(sort_order = nil)
      ...
      #
      #   SELECT  DISTINCT "colibri_products".* FROM "colibri_products" LEFT OUTER JOIN
      #   "colibri_variants" ON "colibri_variants"."product_id" = "colibri_products"."id" AND "colibri_variants"."is_master" = 't'
      #   AND "colibri_variants"."deleted_at" IS NULL LEFT OUTER JOIN "colibri_prices" ON
      #   "colibri_prices"."variant_id" = "colibri_variants"."id" AND "colibri_prices"."currency" = 'USD'
      #   AND "colibri_prices"."deleted_at" IS NULL WHERE "colibri_products"."deleted_at" IS NULL AND ('t'='t') 
      #   ORDER BY "colibri_prices"."amount" ASC LIMIT 10 OFFSET 0
      #
