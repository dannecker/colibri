  ...
  class DefaultTaxZoneValidator < ActiveModel::Validator
    def validate(record)
      if record.included_in_price
        record.errors.add(:included_in_price, Colibri.t(:included_price_validation)) unless Zone.default_tax
      end
    end
  end
end
...
module Colibri
  class TaxRate < Colibri::Base
    acts_as_paranoid
    include Colibri::Core::CalculatedAdjustments
    belongs_to :zone, class_name: "Colibri::Zone"
    ...
    def self.match(order_tax_zone)
      ...
      #
      # Normally, Colibri would notice that you have two potentially applicable
    ...
    # https://github.com/colibri/colibri/issues/4318#issuecomment-34723428
    ...
    def self.default
      ...
      address ||= Address.new(country_id: Colibri::Config[:default_country_id])
    ...
    def adjust(order_tax_zone, item)
      ...
      if amount < 0
        label = Colibri.t(:refund) + ' ' + create_label
      end
