    ...
    module Processing
          ...
          if source
            if !processing?
              if payment_method.supports?(source)
                ...
                invalidate!
                raise Core::GatewayError.new(Colibri.t(:payment_method_not_supported))
              end
            end
          else
            raise Core::GatewayError.new(Colibri.t(:payment_processing_failed))
          end
        end
      end
      ...
      def capture!(amount=nil)
        ...
        protect_from_connection_error do
          ...
          money = ::Money.new(amount, Colibri::Config[:currency])
      ...
      def credit!(credit_amount=nil)
        protect_from_connection_error do
          ...
          credit_cents = Colibri::Money.new(credit_amount, currency: currency).money.cents
      ...
      def gateway_options
        options = { :email       => order.email,
                    ...
                    #
                    # For more information, please see Colibri::Payment#set_unique_identifier
      ...
      def gateway_error(error)
        if error.is_a? ActiveMerchant::Billing::Response
          ...
          text = Colibri.t(:unable_to_connect_to_gateway)
        else
          text = error.to_s
        end
        logger.error(Colibri.t(:gateway_error))
      ...
      def check_environment
        ...
        message = Colibri.t(:gateway_config_unavailable) + " - #{Rails.env}"
