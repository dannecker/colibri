...
module Colibri
  class Shipment < Colibri::Base
    belongs_to :order, class_name: 'Colibri::Order', touch: true, inverse_of: :shipments
    belongs_to :address, class_name: 'Colibri::Address', inverse_of: :shipments
    ...
    def currency
      order ? order.currency : Colibri::Config[:currency]
    end
    ...
    def display_discounted_cost
      Colibri::Money.new(discounted_cost, { currency: currency })
    end

    def display_final_price
      Colibri::Money.new(final_price, { currency: currency })
    end

    def display_item_cost
      Colibri::Money.new(item_cost, { currency: currency })
    end
    ...
    private
      ...
      def description_for_shipping_charge
        "#{Colibri.t(:shipping)} (#{shipping_method.name})"
      end
      ...
      def recalculate_adjustments
        Colibri::ItemAdjustments.new(self).update
      end
