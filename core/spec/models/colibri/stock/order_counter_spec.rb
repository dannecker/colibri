...
module Colibri
  module Stock
    describe OrderCounter do
      let(:variant1) { mock_model(Colibri::Variant) }
      let(:variant2) { mock_model(Colibri::Variant) }

      let(:order) { order = mock_model(Colibri::Order, line_items: [], inventory_units: [])
                    order.line_items << mock_model(Colibri::LineItem, variant: variant1 , quantity: 2)
                    order.line_items << mock_model(Colibri::LineItem, variant: variant2, quantity: 2)
                    order.inventory_units << mock_model(Colibri::InventoryUnit, order: order, variant: variant1)
                    order.inventory_units << mock_model(Colibri::InventoryUnit, order: order, variant: variant2)
                    order.inventory_units << mock_model(Colibri::InventoryUnit, order: order, variant: variant2)
                    order }
