...
module Colibri
  ...
  describe Colibri::ProductDuplicator do
    
    let(:product) { create(:product, properties: [create(:property, name: "MyProperty")])}
    let!(:duplicator) { Colibri::ProductDuplicator.new(product)}
    
    let(:image) { File.open(File.expand_path('../../../fixtures/thinking-cat.jpg', __FILE__)) }
    let(:params) { {:viewable_id => product.master.id, :viewable_type => 'Colibri::Variant', :attachment => image, :alt => "position 1", :position => 1} }
    ...
    it "will duplicate the product" do
      expect{duplicator.duplicate}.to change{Colibri::Product.count}.by(1)
    end

    it "will duplicate the product images" do
      expect{duplicator.duplicate}.to change{Colibri::Image.count}.by(1)
    end
    ...
    context "with variants" do
      ...
      it  "will duplciate the variants" do
        ...
        expect{duplicator.duplicate}.to change{Colibri::Variant.count}.by(3)
      end

      it "will not duplicate the option values" do
        expect{duplicator.duplicate}.to change{Colibri::OptionValue.count}.by(0)
      end
