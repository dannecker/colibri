...
describe Colibri::LegacyUser do
  ...
  context "#last_incomplete_order" do
    ...
    it "returns correct order" do
      user.last_incomplete_colibri_order.should == order_2
    end
    ...
    context "persists order address" do
      it "copies over order addresses" do
        ...
        }.to change { Colibri::Address.count }.by(2)
      ...
      it "doesnt create new addresses if user has already" do
        ...
        expect {
          user.persist_order_address(order)
        }.not_to change { Colibri::Address.count }
      end
...
describe Colibri.user_class do
  context "reporting" do
    ...
    describe "#display_lifetime_value" do
      it "returns a Colibri::Money version of lifetime_value" do
        value = BigDecimal("500.05")
        subject.stub(:lifetime_value).and_return(value)
        expect(subject.display_lifetime_value).to eq Colibri::Money.new(value)
      end
    end
    ...
    describe "#display_average_order_value" do
      before { load_orders }
      it "returns a Colibri::Money version of average_order_value" do
        value = BigDecimal("500.05")
        subject.stub(:average_order_value).and_return(value)
        expect(subject.display_average_order_value).to eq Colibri::Money.new(value)
      end
    end
  end
end
