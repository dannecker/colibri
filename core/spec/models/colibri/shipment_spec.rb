...
describe Colibri::Shipment do
  let(:order) { mock_model Colibri::Order, backordered?: false,
                                         canceled?: false,
                                         can_ship?: true,
                                         currency: 'USD',
                                         touch: true }
  ...
  let(:line_item) { mock_model(Colibri::LineItem, variant: variant) }
  ...
  it 'is backordered if one if its inventory_units is backordered' do
    shipment.stub(inventory_units: [
      mock_model(Colibri::InventoryUnit, backordered?: false),
      mock_model(Colibri::InventoryUnit, backordered?: true)
    ])
    shipment.should be_backordered
  end

  context "display_amount" do
    it "retuns a Colibri::Money" do
      shipment.stub(:cost) { 21.22 }
      shipment.display_amount.should == Colibri::Money.new(21.22)
    end
  end
  ...
  context "display_final_price" do
    it "retuns a Colibri::Money" do
      ...
      shipment.display_final_price.should == Colibri::Money.new(21.22)
    end
  end
  ...
  context "display_item_cost" do
    it "retuns a Colibri::Money" do
      ...
      shipment.display_item_cost.should == Colibri::Money.new(21.22)
    end
  end
  ...
  context 'shipping_rates' do
    ...
    let(:shipping_rates) { [
      Colibri::ShippingRate.new(shipping_method: shipping_method1, cost: 10.00, selected: true),
      Colibri::ShippingRate.new(shipping_method: shipping_method2, cost: 20.00)
    ] }
    ...
    context 'refresh_rates' do
      ...
      it 'should request new rates, and maintain shipping_method selection' do
        Colibri::Stock::Estimator.should_receive(:new).with(shipment.order).and_return(mock_estimator)
      ...
      it 'should not refresh if shipment is shipped' do
        Colibri::Stock::Estimator.should_not_receive(:new)
        shipment.shipping_rates.delete_all
  ...
  context "#update!" do
    ...
    shared_examples_for "pending if backordered" do
      it "should have a state of pending if backordered" do
        shipment.stub(inventory_units: [mock_model(Colibri::InventoryUnit, backordered?: true)])
    ...
    context "when shipment state changes to shipped" do
      ...
      context "with adjustments" do
        before do
          shipment.adjustments << Colibri::Adjustment.create(:label => "Label", :amount => 5)
        end
  ...
  context "when order is completed" do
    ...
    context "with inventory tracking" do
      before { Colibri::Config.set track_inventory_levels: true }
    ...
    context "without inventory tracking" do
      before { Colibri::Config.set track_inventory_levels: false }
  ...
  context "#cancel" do
    ...
    it 'restocks the items' do
      shipment.stub_chain(inventory_units: [mock_model(Colibri::InventoryUnit, state: "on_hand", line_item: line_item, variant: variant)])
      shipment.stock_location = mock_model(Colibri::StockLocation)
  ...
  context "#resume" do
    ...
    it 'unstocks them items' do
      shipment.stub_chain(inventory_units: [mock_model(Colibri::InventoryUnit, line_item: line_item, variant: variant)])
      shipment.stock_location = mock_model(Colibri::StockLocation)
  ...
  context "#ship" do
    ...
    it "should send a shipment email" do
      ...
      shipment_id = nil
      Colibri::ShipmentMailer.should_receive(:shipped_email) { |*args|
        shipment_id = args[0]
        mail_message
      }
  ...
  context "#destroy" do
    it "destroys linked shipping_rates" do
      reflection = Colibri::Shipment.reflect_on_association(:shipping_rates)
