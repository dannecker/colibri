...
describe Colibri::LineItem do
  ...
  context '#destroy' do
    it "fetches deleted products" do
      ...
      expect(line_item.reload.product).to be_a Colibri::Product
    end

    it "fetches deleted variants" do
      line_item.variant.destroy
      expect(line_item.reload.variant).to be_a Colibri::Variant
    end

    it "returns inventory when a line item is destroyed" do
      Colibri::OrderInventory.any_instance.should_receive(:verify).with(line_item, nil)
      line_item.destroy
    end
  end
  ...
  describe ".money" do
    ...
    it "returns a Colibri::Money representing the total for this line item" do
  ...
  describe '.single_money' do
    ...
    it "returns a Colibri::Money representing the price for one variant" do
      line_item.single_money.to_s.should == "$3.50"
    end
  end

  context "has inventory (completed order so items were already unstocked)" do
    let(:order) { Colibri::Order.create(email: 'colibri@example.com') }
    let(:variant) { create(:variant) }
