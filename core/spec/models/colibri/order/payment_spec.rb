...
module Colibri
  describe Colibri::Order do
    ...
    context "ensure source attributes stick around" do
      ...
      # For the reason of this test, please see colibri/colibri_gateway#132
      it "does not have inverse_of defined" do
        expect(Colibri::Order.reflections[:payments].options[:inverse_of]).to be_nil
      end

      it "keeps source attributes after updating" do
        persisted_order = Colibri::Order.create
    ...
    context "#process_payments!" do
      let(:payment) { stub_model(Colibri::Payment) }
      ...
      context "when a payment raises a GatewayError" do
        before { payment.should_receive(:process!).and_raise(Colibri::Core::GatewayError) }

        it "should return true when configured to allow checkout on gateway failures" do
          Colibri::Config.set :allow_checkout_on_gateway_error => true
          order.process_payments!.should be_true
        end

        it "should return false when not configured to allow checkout on gateway failures" do
          Colibri::Config.set :allow_checkout_on_gateway_error => false
          order.process_payments!.should be_false
        end
      end
    end
