...
describe Colibri::Order do
  ...
  context "with default state machine" do
    ...
    it "has the following transitions" do
      transitions.each do |transition|
        transition = Colibri::Order.find_transition(:from => transition.keys.first, :to => transition.values.first)
        transition.should_not be_nil
      end
    end

    it "does not have a transition from delivery to confirm" do
      transition = Colibri::Order.find_transition(:from => :delivery, :to => :confirm)
      transition.should be_nil
    end

    it '.find_transition when contract was broken' do
      Colibri::Order.find_transition({foo: :bar, baz: :dog}).should be_false
    end

    it '.remove_transition' do
      options = {:from => transitions.first.keys.first, :to => transitions.first.values.first}
      Colibri::Order.stub(:next_event_transition).and_return([options])
      Colibri::Order.remove_transition(options).should be_true
    end

    it '.remove_transition when contract was broken' do
      Colibri::Order.remove_transition(nil).should be_false
    end
    ...
    it "cannot transition to address without any line items" do
      ...
      lambda { order.next! }.should raise_error(StateMachine::InvalidTransition, /#{Colibri.t(:there_are_no_items_for_this_order)}/)
    end
        ...
        context "with an existing shipment" do
          ...
          context "if there are no shipping rates for any shipment" do
            it "raises an InvalidTransitionError" do
              ...
              transition.should raise_error(StateMachine::InvalidTransition, /#{Colibri.t(:items_cannot_be_shipped)}/)
            end
  ...
  context "subclassed order" do
    ...
    #   the other test pass
    class SubclassedOrder < Colibri::Order
      checkout_flow do
        go_to_state :payment
  ...
  context "re-define checkout flow" do
    ...
    it "should not keep old event transitions when checkout_flow is redefined" do
      Colibri::Order.next_event_transitions.should == [{:cart=>:payment}, {:payment=>:complete}]
    end

    it "should not keep old events when checkout_flow is redefined" do
      state_machine = Colibri::Order.state_machine
  ...
  describe "payment processing" do
    ...
    it "does not process payment within transaction" do
      ...
      Colibri::Payment.any_instance.should_receive(:authorize!) do
        ActiveRecord::Base.connection.open_transactions.should == 0
      end
  ...
  describe 'update_from_params' do
    ...
    context "passing a credit card" do
      let(:permitted_params) do
        Colibri::PermittedAttributes.checkout_attributes +
          [payments_attributes: Colibri::PermittedAttributes.payment_attributes]
      end
      ...
      it "sets existing card as source for new payment" do
        expect {
          order.update_from_params(params, permitted_params)
        }.to change { Colibri::Payment.count }.by(1)

        expect(Colibri::Payment.last.source).to eq credit_card
      end
