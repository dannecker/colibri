...
describe Colibri::InventoryUnit do
  ...
  context "#backordered_for_stock_item" do
    ...
    it "returns modifiable objects" do
      units = Colibri::InventoryUnit.backordered_for_stock_item(stock_item)
    ...
    it "finds inventory units from its stock location when the unit's variant matches the stock item's variant" do
      Colibri::InventoryUnit.backordered_for_stock_item(stock_item).should =~ [unit]
    end
    ...
    it "does not find inventory units that aren't backordered" do
      ...
      Colibri::InventoryUnit.backordered_for_stock_item(stock_item).should_not include(on_hand_unit)
    end
    ...
    it "does not find inventory units that don't match the stock item's variant" do
      ...
      Colibri::InventoryUnit.backordered_for_stock_item(stock_item).should_not include(other_variant_unit)
    end
    ...
    context "other shipments" do
      ...
      it "does not find inventory units belonging to incomplete orders" do
        Colibri::InventoryUnit.backordered_for_stock_item(stock_item).should_not include(other_unit)
      end
  ...
  context "variants deleted" do
    let!(:unit) do
      Colibri::InventoryUnit.create(variant: stock_item.variant)
    end

    it "can still fetch variant" do
      unit.variant.destroy
      expect(unit.reload.variant).to be_a Colibri::Variant
    end
    ...
    it "can still fetch variants by eager loading (remove default_scope)" do
      ...
      unit.variant.destroy
      expect(Colibri::InventoryUnit.joins(:variant).includes(:variant).first.variant).to be_a Colibri::Variant
    end
  end
  ...
  context "#finalize_units!" do
    ...
    it "should create a stock movement" do
      Colibri::InventoryUnit.finalize_units!(inventory_units)
