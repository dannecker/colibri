...
describe Colibri::Promotion do
  let(:promotion) { Colibri::Promotion.new }

  describe "validations" do
    before :each do
      @valid_promotion = Colibri::Promotion.new :name => "A promotion"
    end
  ...
  describe ".advertised" do
    ...
    it "only shows advertised promotions" do
      advertised = Colibri::Promotion.advertised
  ...
  describe "#destroy" do
    let(:promotion) { Colibri::Promotion.create(:name => "delete me") }

    before(:each) do
      promotion.actions << Colibri::Promotion::Actions::CreateAdjustment.new
      promotion.rules << Colibri::Promotion::Rules::FirstOrder.new
      promotion.save!
      promotion.destroy
    end

    it "should delete actions" do
      Colibri::PromotionAction.count.should == 0
    end

    it "should delete rules" do
      Colibri::PromotionRule.count.should == 0
    end
  end
  ...
  describe "#save" do
    ...
    it "should deeply autosave records and preferences" do
      ...
      promotion.save!
      Colibri::Calculator.first.preferred_flat_percent.should == 10
    end
  end

  describe "#activate" do
    before do
      @action1 = Colibri::Promotion::Actions::CreateAdjustment.create!
      @action2 = Colibri::Promotion::Actions::CreateAdjustment.create!
      ...
      @user = stub_model(Colibri::LegacyUser, :email => "colibri@example.com")
      @order = Colibri::Order.create user: @user
  ...
  context "#credits_count" do
    ...
    let!(:action) do
      ...
      action = Colibri::Promotion::Actions::CreateAdjustment.create(action_params)
      promotion.actions << action
      action
    end

    let!(:adjustment) do
      Colibri::Adjustment.create!(
        :source => action,
        :amount => 10,
  ...
  context "#products" do
    ...
    context "when it has product rules with products associated" do
      let(:promotion_rule) { Colibri::Promotion::Rules::Product.new }
    ...
    context "when there's no product rule associated" do
      before(:each) do
        promotion.stub_chain(:rules, :to_a).and_return([mock_model(Colibri::Promotion::Rules::User)])
      end
  ...
  context "#rules_are_eligible?" do
    ...
    it "true if there are no applicable rules" do
      promotion.promotion_rules = [stub_model(Colibri::PromotionRule, :eligible? => true, :applicable? => false)]
    ...
    context "with 'all' match policy" do
      ...
      it "should have eligible rules if all rules are eligible" do
        promo1 = Colibri::PromotionRule.create!
        promo1.stub(eligible?: true, applicable?: true)
        promo2 = Colibri::PromotionRule.create!
    ...
    context "with 'any' match policy" do
      let(:promotion) { Colibri::Promotion.create(:name => "Promo", :match_policy => 'any') }
      let(:promotable) { double('Promotable') }

      it "should have eligible rules if any of the rules are eligible" do
        Colibri::PromotionRule.any_instance.stub(:applicable? => true)
        true_rule = Colibri::PromotionRule.create(:promotion => promotion)
  ...
  describe "normalize blank values for code & path" do
    it "will save blank value as nil value instead" do
      promotion = Colibri::Promotion.create(:name => "A promotion", :code => "", :path => "")
  ...
  describe "#with_coupon_code" do
    context "and code stored in uppercase" do
      ...
      it "finds the code with lowercase" do
        expect(Colibri::Promotion.with_coupon_code("my-coupon-123")).to eql promotion
      end
    end
  end
end
