...
module ThirdParty
  class Extension < Colibri::Base
    # nasty hack so we don't have to create a table to back this fake model
    self.table_name = 'colibri_products'
  end
end

describe Colibri::Product do
  context 'product instance' do
    ...
    context "#display_price" do
      ...
      context "with display_currency set to true" do
        before { Colibri::Config[:display_currency] = true }
      ...
      context "with display_currency set to false" do
        before { Colibri::Config[:display_currency] = false }
    ...
    describe 'Variants sorting' do
      context 'without master variant' do
        it 'sorts variants by position' do
          product.variants.to_sql.should match(/ORDER BY (\`|\")colibri_variants(\`|\").position ASC/)
        end
      end

      context 'with master variant' do
        it 'sorts variants by position' do
          product.variants_including_master.to_sql.should match(/ORDER BY (\`|\")colibri_variants(\`|\").position ASC/)
        end
      end
    end
    ...
    context "has stock movements" do
      ...
      it "doesnt raise ReadOnlyRecord error" do
        Colibri::StockMovement.create!(stock_item: stock_item, quantity: 1)
  ...
  context "properties" do
    ...
    it "should not overwrite properties' presentation names" do
      Colibri::Property.where(:name => 'foo').first_or_create!(:presentation => "Foo's Presentation Name")
      product.set_property('foo', 'value1')
      product.set_property('bar', 'value2')
      Colibri::Property.where(:name => 'foo').first.presentation.should == "Foo's Presentation Name"
      Colibri::Property.where(:name => 'bar').first.presentation.should == "bar"
    end
    ...
    context "#possible_promotions" do
      ...
      let!(:rule) do
        Colibri::Promotion::Rules::Product.create(
          promotion: promotion,
          products: [product]
        )
      end
  ...
  context '#create' do
    ...
    let!(:product) { Colibri::Product.new(name: "Foo", price: 1.99, shipping_category_id: create(:shipping_category).id) }
  ...
  context "#images" do
    ...
    let(:params) { {:viewable_id => product.master.id, :viewable_type => 'Colibri::Variant', :attachment => image, :alt => "position 2", :position => 2} }

    before do
      Colibri::Image.create(params)
      Colibri::Image.create(params.merge({:alt => "position 1", :position => 1}))
      Colibri::Image.create(params.merge({:viewable_type => 'ThirdParty::Extension', :alt => "position 1", :position => 2}))
    end
  ...
  context "classifications and taxons" do
    it "is joined through classifications" do
      reflection = Colibri::Product.reflect_on_association(:taxons)
      expect(reflection.options[:through]).to eq(:classifications)
    end

    it "will delete all classifications" do
      reflection = Colibri::Product.reflect_on_association(:classifications)
  ...
  describe '#total_on_hand' do
    ...
    it 'should return sum of stock items count_on_hand' do
      ...
      product.stub stock_items: [double(Colibri::StockItem, count_on_hand: 5)]
  ...
  describe "slugs" do
    it "normalizes slug on update" do
      product = stub_model Colibri::Product
