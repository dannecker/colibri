...
describe Colibri::CreditCard do
  ...
  def self.payment_states
    Colibri::Payment.state_machine.states.keys
  end
  ...
  let(:credit_card) { Colibri::CreditCard.new }

  before(:each) do

    @order = create(:order)
    @payment = Colibri::Payment.create(:amount => 100, :order => @order)
    ...
    @payment_gateway = mock_model(Colibri::PaymentMethod,
  ...
  context "#can_capture?" do
    it "should be true if payment is pending" do
      payment = mock_model(Colibri::Payment, pending?: true, created_at: Time.now)
      credit_card.can_capture?(payment).should be_true
    end

    it "should be true if payment is checkout" do
      payment = mock_model(Colibri::Payment, pending?: false, checkout?: true, created_at: Time.now)
      credit_card.can_capture?(payment).should be_true
    end
  end
  ...
  context "#can_void?" do
    it "should be true if payment is not void" do
      payment = mock_model(Colibri::Payment, void?: false)
  ...
  context "#can_credit?" do
    it "should be false if payment is not completed" do
      payment = mock_model(Colibri::Payment, completed?: false)
      credit_card.can_credit?(payment).should be_false
    end

    it "should be false when order payment_state is not 'credit_owed'" do
      payment = mock_model(Colibri::Payment, completed?: true, order: mock_model(Colibri::Order, payment_state: 'paid'))
      credit_card.can_credit?(payment).should be_false
    end

    it "should be false when credit_allowed is zero" do
      payment = mock_model(Colibri::Payment, completed?: true, credit_allowed: 0, order: mock_model(Colibri::Order, payment_state: 'credit_owed'))
      credit_card.can_credit?(payment).should be_false
    end
  end
  ...
  context "#save" do
    ...
    let!(:persisted_card) { Colibri::CreditCard.find(credit_card.id) }
