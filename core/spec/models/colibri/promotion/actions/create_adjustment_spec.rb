...
describe Colibri::Promotion::Actions::CreateAdjustment do
  let(:order) { create(:order_with_line_items, :line_items_count => 1) }
  let(:promotion) { create(:promotion) }
  let(:action) { Colibri::Promotion::Actions::CreateAdjustment.new }

  # From promotion spec:
  context "#perform" do
    before do
      action.calculator = Colibri::Calculator::FlatRate.new(:preferred_amount => 10)
