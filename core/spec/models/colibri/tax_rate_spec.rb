...
describe Colibri::TaxRate do
  context "match" do
    let(:order) { create(:order) }
    let(:country) { create(:country) }
    let(:tax_category) { create(:tax_category) }
    let(:calculator) { Colibri::Calculator::FlatRate.new }

    it "should return an empty array when tax_zone is nil" do
      order.stub :tax_zone => nil
      Colibri::TaxRate.match(order.tax_zone).should == []
    end

    context "when no rate zones match the tax zone" do
      before do
        Colibri::TaxRate.create(:amount => 1, :zone => create(:zone))
      end
      ...
      context "when there is no default tax zone" do
        ...
        it "should return the rate that matches the rate zone" do
          ...
          order.stub :tax_zone => @zone
          Colibri::TaxRate.match(order.tax_zone).should == [rate]
        end

        it "should return all rates that match the rate zone" do
          rate1 = Colibri::TaxRate.create(
            :amount => 1,
            :zone => @zone,
            :tax_category => tax_category,
            :calculator => calculator
          ...
          order.stub :tax_zone => @zone
          Colibri::TaxRate.match(order.tax_zone).should match_array([rate1, rate2])
        end
      ...
      context "when there is a default tax zone" do
        ...
        let!(:rate) do
          Colibri::TaxRate.create(:amount => 1,
                                :zone => @zone,
                                :tax_category => tax_category,
                                :calculator => calculator,
        ...
        subject { Colibri::TaxRate.match(order.tax_zone) }
        ...
        context "when the order has the same tax zone" do
          ...
          let(:tax_address) { stub_model(Colibri::Address) }
  ...
  context "adjust" do
    ...
    let(:tax_category_1) { stub_model(Colibri::TaxCategory) }
    let(:tax_category_2) { stub_model(Colibri::TaxCategory) }
    let(:rate_1) { stub_model(Colibri::TaxRate, :tax_category => tax_category_1) }
    let(:rate_2) { stub_model(Colibri::TaxRate, :tax_category => tax_category_2) }

    context "with line items" do
      let(:line_item) do
        stub_model(Colibri::LineItem, 
          :tax_category => tax_category_1,
          :variant => stub_model(Colibri::Variant)
        )
      end
      ...
      before do
        Colibri::TaxRate.stub :match => [rate_1, rate_2]
      end
      ...
      it "should apply adjustments for two tax rates to the order" do
        ...
        Colibri::TaxRate.adjust(order.tax_zone, line_items)
      end
    end

    context "with shipments" do
      let(:shipments) { [stub_model(Colibri::Shipment, :tax_category => tax_category_1)] }
      ...
      it "should apply adjustments for two tax rates to the order" do
        rate_1.should_receive(:adjust)
        rate_2.should_not_receive(:adjust)
        Colibri::TaxRate.adjust(order.tax_zone, shipments)
      end
    end
  end
  ...
  context "default" do
    ...
    context "when there is no default tax_category" do
      ...
      it "should return 0" do
        Colibri::TaxRate.default.should == 0
      end
    end
    ...
    context "when there is a default tax_category" do
      ...
      context "when the default category has tax rates in the default tax zone" do
        ...
        it "should return the correct tax_rate" do
          Colibri::TaxRate.default.to_f.should == 1.0
        end
      end
  ...
  context "#adjust" do
    before do
      ...
      @zone = create(:zone, :name => "Country Zone", :default_tax => true, :zone_members => [])
      @zone.zone_members.create(:zoneable => @country)
      @category    = Colibri::TaxCategory.create :name => "Taxable Foo"
      @category2   = Colibri::TaxCategory.create(:name => "Non Taxable")
      @rate1        = Colibri::TaxRate.create(
        :amount => 0.10,
        :calculator => Colibri::Calculator::DefaultTax.create,
    ...
    context "not taxable line item " do
      ...
      it "should not create a tax adjustment" do
        Colibri::TaxRate.adjust(@order.tax_zone, @order.line_items)
    ...
    context "taxable line item" do
      ...
      context "when price includes tax" do
        before do
          ...
          Colibri::TaxRate.store_pre_tax_amount(line_item, [@rate1, @rate2])
        end
