...
class FakeCalculator < Colibri::Calculator
...
describe Colibri::Order do
  let(:user) { stub_model(Colibri::LegacyUser, :email => "colibri@example.com") }
  let(:order) { stub_model(Colibri::Order, :user => user) }

  before do
    Colibri::LegacyUser.stub(:current => mock_model(Colibri::LegacyUser, :id => 123))
  end

  context "#create" do
    it "should assign an order number" do
      order = Colibri::Order.create
  ...
  context "#finalize!" do
    let(:order) { Colibri::Order.create(email: 'test@example.com') }
    ...
    it "should change the shipment state to ready if order is paid" do
      Colibri::Shipment.create(order: order)
    ...
    after { Colibri::Config.set :track_inventory_levels => true }
    it "should not sell inventory units if track_inventory_levels is false" do
      Colibri::Config.set :track_inventory_levels => false
      Colibri::InventoryUnit.should_not_receive(:sell_units)
      order.finalize!
    end

    it "should send an order confirmation email" do
      mail_message = double "Mail::Message"
      Colibri::OrderMailer.should_receive(:confirm_email).with(order.id).and_return mail_message
    ...
    it "should not send duplicate confirmation emails" do
      ...
      Colibri::OrderMailer.should_not_receive(:confirm_email)
      order.finalize!
    end
    ...
    it "should freeze all adjustments" do
      ...
      Colibri::OrderMailer.stub_chain :confirm_email, :deliver
      adjustments = [double]
  ...
  context "insufficient_stock_lines" do
    let(:line_item) { mock_model Colibri::LineItem, :insufficient_stock? => true }
  ...
  context "empty!" do
    let(:order) { stub_model(Colibri::Order, item_count: 2) }
  ...
  context "#display_outstanding_balance" do
    it "returns the value as a colibri money" do
      order.stub(:outstanding_balance) { 10.55 }
      order.display_outstanding_balance.should == Colibri::Money.new(10.55)
    end
  end
  ...
  context "#display_item_total" do
    it "returns the value as a colibri money" do
      ...
      order.display_item_total.should == Colibri::Money.new(10.55)
    end
  end
  ...
  context "#display_adjustment_total" do
    it "returns the value as a colibri money" do
      ...
      order.display_adjustment_total.should == Colibri::Money.new(10.55)
    end
  end
  ...
  context "#display_total" do
    it "returns the value as a colibri money" do
      order.total = 10.55
      order.display_total.should == Colibri::Money.new(10.55)
    end
  end
  ...
  context "#merge!" do
    ...
    let(:order_1) { Colibri::Order.create }
    let(:order_2) { Colibri::Order.create }
  ...
  context "#confirmation_required?" do
    ...
    context 'Colibri::Config[:always_include_confirm_step] == true' do

      before do
        Colibri::Config[:always_include_confirm_step] = true
      end
    ...
    context 'Colibri::Config[:always_include_confirm_step] == false' do

      it "returns false if payments empty and Colibri::Config[:always_include_confirm_step] == false" do
        order = Colibri::Order.new
        assert !order.confirmation_required?
      end
  ...
  context "add_update_hook" do
    ...
    after do
      Colibri::Order.update_hooks = Set.new
    end
  ...
  describe "#tax_address" do
    before { Colibri::Config[:tax_using_ship_address] = tax_using_ship_address }
  ...
  context "#available_payment_methods" do
    it "includes frontend payment methods" do
      payment_method = Colibri::PaymentMethod.create!({
        :name => "Fake",
        :active => true,
  ...
  context "#apply_free_shipping_promotions" do
    it "calls out to the FreeShipping promotion handler" do
      ...
      Colibri::PromotionHandler::FreeShipping.should_receive(:new).and_return(handler = double)
      handler.should_receive(:activate)

      Colibri::ItemAdjustments.should_receive(:new).with(shipment).and_return(adjuster = double)
  ...
  context "#products" do
    before :each do
      @variant1 = mock_model(Colibri::Variant, :product => "product1")
      @variant2 = mock_model(Colibri::Variant, :product => "product2")
      @line_items = [mock_model(Colibri::LineItem, :product => "product1", :variant => @variant1, :variant_id => @variant1.id, :quantity => 1),
                     mock_model(Colibri::LineItem, :product => "product2", :variant => @variant2, :variant_id => @variant2.id, :quantity => 2)]
    ...
    it "gets the quantity of a given variant" do
      ...
      @variant3 = mock_model(Colibri::Variant, :product => "product3")
      order.quantity_of(@variant3).should == 0
    end

    it "can find a line item matching a given variant" do
      order.find_line_item_by_variant(@variant1).should_not be_nil
      order.find_line_item_by_variant(mock_model(Colibri::Variant)).should be_nil
    end
  end
  ...
  context "#backordered?" do
    it 'is backordered if one of the shipments is backordered' do
      order.stub(:shipments => [mock_model(Colibri::Shipment, :backordered? => false),
                                mock_model(Colibri::Shipment, :backordered? => true)])
