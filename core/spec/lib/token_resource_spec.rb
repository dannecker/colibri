...
describe Colibri::Core::TokenResource do
  let(:order) { Colibri::Order.new }
  let(:permission) { mock_model(Colibri::TokenizedPermission) }

  it 'should add has_one :tokenized_permission relationship' do
    assert Colibri::Order.reflect_on_all_associations(:has_one).map(&:name).include?(:tokenized_permission)
  end
