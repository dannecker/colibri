...
describe Colibri::Core::Search::Base do

  before do
    include Colibri::Core::ProductFilters
    Colibri::Product.delete_all # FIXME product leaks
  ...
  it "returns all products by default" do
    params = { :per_page => "" }
    searcher = Colibri::Core::Search::Base.new(params)
  ...
  it "accepts a current user" do
    user = double
    searcher = Colibri::Core::Search::Base.new({})
