...
module Colibri
  describe Migrations do
    ...
    let(:engine_migrations) { [".", "334_create_orders.colibri.rb", "777_create_products.colibri.rb"] }

    let(:config) { double("Config", root: "dir") }

    subject { described_class.new(config, "colibri")  }

    before do
      expect(File).to receive(:exists?).with("config/colibri.yml").and_return true
