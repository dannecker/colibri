...
describe Colibri::BaseHelper do
  include Colibri::BaseHelper
  ...
  context "available_countries" do
    ...
    context "with no checkout zone defined" do
      before do
        Colibri::Config[:checkout_zone] = nil
      end

      it "return complete list of countries" do
        available_countries.count.should == Colibri::Country.count
      end
    end
    ...
    context "with a checkout zone defined" do
      context "checkout zone is of type country" do
        before do
          ...
          Colibri::Config[:checkout_zone] = @country_zone.name
        end
      ...
      context "checkout zone is of type state" do
        before do
          ...
          Colibri::Config[:checkout_zone] = state_zone.name
        end
  ...
  context "defining custom image helpers" do
    let(:product) { mock_model(Colibri::Product, :images => [], :variant_images => []) }
    before do
      Colibri::Image.class_eval do
  ...
  context "meta_data_tags" do
    it "truncates a product description to 160 characters" do
      ...
      @test = Colibri::Product.new(:description => text)
