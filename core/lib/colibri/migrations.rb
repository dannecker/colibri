module Colibri
  class Migrations
    ...
    # First split:
    #
    #   ["20131128203548", "update_name_fields_on_colibri_credit_cards.colibri.rb"]
    #
    # Second split should give the engine_name of the migration
    #
    #   ["update_name_fields_on_colibri_credit_cards", "colibri.rb"]
    #
    # Shouldn't run on test mode because migrations inside engine don't have
    # engine name on the file name
    def check
      if File.exists?("config/colibri.yml") && File.directory?("db/migrate")
    ...
    private
      ...
      def match_engine?(engine)
        if engine_name == "colibri"
          # Avoid stores upgrading from 1.3 getting wrong warnings
          ["colibri.rb", "colibri_promo.rb"].include? engine
        else
