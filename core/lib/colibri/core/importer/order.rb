      ...
      class Order
        ...
        def self.create_shipments_from_params(shipments_hash, order)
          ...
          shipments_hash.each do |s|
            begin
              ...
              shipment.stock_location = Colibri::StockLocation.find_by_name!(s[:stock_location])
              ...
              shipping_method = Colibri::ShippingMethod.find_by_name!(s[:shipping_method])
        ...
        def self.create_line_items_from_params(line_items_hash, order)
          ...
          line_items_hash.each_key do |k|
            begin
              ...
              line_item = order.contents.add(Colibri::Variant.find(line_item[:variant_id]), line_item[:quantity])
        ...
        def self.create_payments_from_params(payments_hash, order)
          ...
          payments_hash.each do |p|
            begin
              ...
              payment.payment_method = Colibri::PaymentMethod.find_by_name!(p[:payment_method])
              payment.save!
            rescue Exception => e
        ...
        def self.ensure_variant_id_from_params(hash)
          begin
            unless hash[:variant_id].present?
              hash[:variant_id] = Colibri::Variant.active.find_by_sku!(hash[:sku]).id
              hash.delete(:sku)
            end
          rescue Exception => e
        ...
        def self.ensure_country_id_from_params(address)
          ...
          begin
            ...
            address[:country_id] = Colibri::Country.where(search).first!.id
        ...
        def self.ensure_state_id_from_params(address)
          ...
          begin
            ...
            if state = Colibri::State.where(search).first
