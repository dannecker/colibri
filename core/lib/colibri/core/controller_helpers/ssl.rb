  ...
  module Core
    module ControllerHelpers
      module SSL
        ...
        included do
          before_filter :force_non_ssl_redirect, :if => Proc.new { Colibri::Config[:redirect_https_to_http] }
          ...
          def ssl_supported?
            return Colibri::Config[:allow_ssl_in_production] if Rails.env.production?
            return Colibri::Config[:allow_ssl_in_staging] if Rails.env.staging?
            return Colibri::Config[:allow_ssl_in_development_and_test] if (Rails.env.development? or Rails.env.test?)
          end
            ...
            def force_non_ssl_redirect(host = nil)
              if request.ssl? && !ssl_allowed?
                if request.get?
                  ...
                  render :text => Colibri.t(:change_protocol, :scope => :ssl), :status => :upgrade_required
                end
              end
            end
        end
