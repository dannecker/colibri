module Colibri
  module Core
    ...
    module ProductFilters
      ...
      #
      Colibri::Product.add_search_scope :price_range_any do |*opts|
        conds = opts.map {|o| Colibri::Core::ProductFilters.price_filter[:conds][o]}.reject { |c| c.nil? }
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Colibri::Product.joins(master: :default_price).where(scope)
      end

      def ProductFilters.format_price(amount)
        Colibri::Money.new(amount)
      end

      def ProductFilters.price_filter
        v = Colibri::Price.arel_table
        conds = [ [ Colibri.t(:under_price, price: format_price(10))     , v[:amount].lteq(10)],
                  ...
                  [ Colibri.t(:or_over_price, price: format_price(20)) , v[:amount].gteq(20)]]
        {
          name:   Colibri.t(:price_range),
      ...
      Colibri::Product.add_search_scope :brand_any do |*opts|
        conds = opts.map {|o| ProductFilters.brand_filter[:conds][o]}.reject { |c| c.nil? }
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Colibri::Product.with_property('brand').where(scope)
      end

      def ProductFilters.brand_filter
        brand_property = Colibri::Property.find_by(name: 'brand')
        brands = brand_property ? Colibri::ProductProperty.where(property_id: brand_property.id).pluck(:value).uniq.map(&:to_s) : []
        pp = Colibri::ProductProperty.arel_table
      ...
      Colibri::Product.add_search_scope :selective_brand_any do |*opts|
        Colibri::Product.brand_any(*opts)
      end

      def ProductFilters.selective_brand_filter(taxon = nil)
        taxon ||= Colibri::Taxonomy.first.root
        brand_property = Colibri::Property.find_by(name: 'brand')
        scope = Colibri::ProductProperty.where(property: brand_property).
          joins(product: :taxons).
          where("#{Colibri::Taxon.table_name}.id" => [taxon] + taxon.descendants).
          scoped
      ...
      def ProductFilters.taxons_below(taxon)
        return Colibri::Core::ProductFilters.all_taxons if taxon.nil?
        {
      ...
      def ProductFilters.all_taxons
        taxons = Colibri::Taxonomy.all.map { |t| [t.root] + t.root.descendants }.flatten
        {
          name:   'All taxons',
