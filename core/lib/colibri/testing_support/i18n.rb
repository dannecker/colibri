...
# within the code that Colibri's test suite covers.
#
...
module Colibri
    ...
    used_translations.map { |a| a.split('.') }.each do |translation_keys|
      ...
      translation_keys.each do |key|
        begin
          ...
          unless Colibri.missing_translation_messages.include?(error)
            Colibri.missing_translation_messages << error
          end
        end
      end
    end
  ...
  def self.check_unused_translations
    ...
    translation_diff.each do |translation|
      Colibri.unused_translation_messages << "#{translation} (#{I18n.locale})"
    end
  end
  ...
  def self.translations
    @translations ||= I18n.backend.send(:translations)[I18n.locale][:colibri]
  end
end
...
RSpec.configure do |config|
  ...
  if ENV['CHECK_TRANSLATIONS']
    config.after :suite do
      ...
      if Colibri.missing_translation_messages.any?
        puts "\nThere are missing translations within Colibri:"
        puts Colibri.missing_translation_messages.sort
        exit(1)
      end

      Colibri.check_unused_translations
      if false && Colibri.unused_translation_messages.any?
        puts "\nThere are unused translations within Colibri:"
        puts Colibri.unused_translation_messages.sort
        exit(1)
      end
    end
  end
