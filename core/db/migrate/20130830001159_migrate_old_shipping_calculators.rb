class MigrateOldShippingCalculators < ActiveRecord::Migration
  def up
    Colibri::ShippingMethod.all.each do |shipping_method|
      old_calculator = shipping_method.calculator
      next if old_calculator.class < Colibri::ShippingCalculator # We don't want to mess with new shipping calculators
