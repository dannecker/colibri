...
describe 'Users' do
  ...
  shared_examples_for 'a user page' do
    it 'has lifetime stats' do
      ...
      within("#user-lifetime-stats") do
        [:total_sales, :num_orders, :average_order_value, :member_since].each do |stat_name|
          expect(page).to have_content Colibri.t(stat_name)
        end
    ...
    it 'can go back to the users list' do
      expect(page).to have_link Colibri.t(:back_to_users_list), href: colibri.admin_users_path
    end

    it 'can navigate to the account page' do
      expect(page).to have_link Colibri.t(:"admin.user.account"), href: colibri.edit_admin_user_path(user_a)
    end

    it 'can navigate to the order history' do
      expect(page).to have_link Colibri.t(:"admin.user.orders"), href: colibri.orders_admin_user_path(user_a)
    end

    it 'can navigate to the items purchased' do
      expect(page).to have_link Colibri.t(:"admin.user.items"), href: colibri.items_admin_user_path(user_a)
    end
  end
  ...
  context 'editing users' do
    ...
    it 'can edit user roles' do
      Colibri::Role.create name: "admin"
      click_link user_a.email

      check 'user_colibri_role_admin'
      click_button 'Update'
      expect(page).to have_text 'Account updated'
      expect(find_field('user_colibri_role_admin')['checked']).to be_true
    end
    ...
    context 'no api key exists' do
      it 'can generate a new api key' do
        within("#admin_user_edit_api_key") do
          expect(user_a.colibri_api_key).to be_blank
          click_button Colibri.t('generate_key', :scope => 'api')
        end

        expect(user_a.reload.colibri_api_key).to be_present

        within("#admin_user_edit_api_key") do
          expect(find("#current-api-key").text).to match /Key: #{user_a.colibri_api_key}/
        end
      end
    end
    ...
    context 'an api key exists' do
      before do
        user_a.generate_colibri_api_key!
        expect(user_a.reload.colibri_api_key).to be_present
        visit current_path
      end
      ...
      it 'can clear an api key' do
        within("#admin_user_edit_api_key") do
          click_button Colibri.t('clear_key', :scope => 'api')
        end

        expect(user_a.reload.colibri_api_key).to be_blank
        expect { find("#current-api-key") }.to raise_error Capybara::ElementNotFound
      end

      it 'can regenerate an api key' do
        old_key = user_a.colibri_api_key

        within("#admin_user_edit_api_key") do
          click_button Colibri.t('regenerate_key', :scope => 'api')
        end

        expect(user_a.reload.colibri_api_key).to be_present
        expect(user_a.reload.colibri_api_key).not_to eq old_key
  ...
  context 'order history with sorting' do
    ...
    before do
      orders
      click_link user_a.email
      within("#sidebar") { click_link Colibri.t(:"admin.user.orders") }
    end
  ...
  context 'items purchased with sorting' do
    ...
    before do
      orders
      click_link user_a.email
      within("#sidebar") { click_link Colibri.t(:"admin.user.items") }
    end
