...
describe "Orders Listing" do
  ...
  context "searching orders" do
    ...
    context "when pagination is really short" do
      before do
        @old_per_page = Colibri::Config[:orders_per_page]
        Colibri::Config[:orders_per_page] = 1
      end

      after do
        Colibri::Config[:orders_per_page] = @old_per_page
      end

      # Regression test for #4004
      it "should be able to go from page to page for incomplete orders" do
        10.times { Colibri::Order.create :email => "incomplete@example.com" }
