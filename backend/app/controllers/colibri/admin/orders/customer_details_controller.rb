module Colibri
  module Admin
    module Orders
      class CustomerDetailsController < Colibri::Admin::BaseController
        ...
        def update
          if @order.update_attributes(order_params)
            if params[:guest_checkout] == "false"
              @order.associate_user!(Colibri.user_class.find_by_email(@order.email))
            end
            while @order.next; end

            @order.refresh_shipment_rates
            flash[:success] = Colibri.t('customer_details_updated')
