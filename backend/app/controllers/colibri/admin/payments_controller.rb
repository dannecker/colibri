module Colibri
  module Admin
    class PaymentsController < Colibri::Admin::BaseController
      ...
      def create
        ...
        begin
          if @payment.save
            ...
            flash[:error] = Colibri.t(:payment_could_not_be_created)
            render :new
          end
        rescue Colibri::Core::GatewayError => e
          invoke_callbacks(:create, :fails)
      ...
      def fire
        ...
        if @payment.send("#{event}!")
          flash[:success] = Colibri.t(:payment_updated)
        else
          flash[:error] = Colibri.t(:cannot_perform_operation)
        end
      rescue Colibri::Core::GatewayError => ge
      ...
      def can_transition_to_payment
        unless @order.billing_address.present?
          flash[:notice] = Colibri.t(:fill_in_customer_info)
