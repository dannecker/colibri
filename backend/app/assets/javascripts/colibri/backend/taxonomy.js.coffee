...
handle_move = (e, data) ->
  ...
  url = Colibri.url(base_url).clone()
...
handle_delete = (e, data) ->
  ...
  jConfirm Colibri.translations.are_you_sure_delete, Colibri.translations.confirm_delete, (r) ->
    if r
      $.ajax
        type: "POST",
        dataType: "json",
...
root.setup_taxonomy_tree = (taxonomy_id) ->
  if taxonomy_id != undefined
    ...
    root.base_url = Colibri.url(Colibri.routes.taxonomy_taxons_path)

    $.ajax
      url: Colibri.url(base_url.path().replace("/taxons", "/jstree")).toString(),
      success: (taxonomy) ->
        last_rollback = null
        ...
        conf =
          json_data:
            ...
            ajax:
              url: (e) ->
                Colibri.url(base_url.path() + '/' + e.prop('id') + '/jstree').toString()
          themes:
            theme: "apple",
            url: Colibri.url(Colibri.routes.jstree_theme_path)
          strings:
            new_node: new_taxon,
            loading: Colibri.translations.loading + "..."
          crrm:
            move:
              check_move: (m) ->
                position = m.cp
