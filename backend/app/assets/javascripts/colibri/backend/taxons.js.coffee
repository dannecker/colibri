$(document).ready ->
  ...
  $('#taxon_products').on "sortstop", (event, ui) ->
    $.ajax
      url: Colibri.routes.classifications_api,
      method: 'PUT',
      data:
  ...
  if $('#taxon_id').length > 0
    $('#taxon_id').select2
      ...
      placeholder: Colibri.translations.find_a_taxon,
      ajax:
  ...
  $('#taxon_id').on "change", (e) ->
    el = $('#taxon_products')
    $.ajax
      url: Colibri.routes.taxon_products_api,
      data:
        id: e.val
      success: (data) ->
        el.empty();
        if data.products.length == 0
          ...
          $('#taxon_products').html("<h4>" + Colibri.translations.no_results + "</h4>")
        else
