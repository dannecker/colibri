  ...
  module Admin
    module OrdersHelper
      ...
      def event_links
        ...
        @order_events.sort.each do |event|
          if @order.send("can_#{event}?")
            links << button_link_to(Colibri.t(event), [event, :admin, @order],
                                    :method => :put,
                                    :icon => "icon-#{event}",
                                    :data => { :confirm => Colibri.t(:order_sure_want_to, :event => Colibri.t(event)) })
          end
        end
      ...
      def line_item_shipment_price(line_item, quantity)
        Colibri::Money.new(line_item.price * quantity, { currency: line_item.currency })
      end
