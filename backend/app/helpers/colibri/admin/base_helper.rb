  ...
  module Admin
    module BaseHelper
      ...
      def datepicker_field_value(date)
        unless date.blank?
          l(date, :format => Colibri.t('date_picker.format', default: '%Y/%m/%d'))
        else
          nil
        end
      end
      ...
      def preference_fields(object, form)
        ...
        object.preferences.keys.map{ |key|
          ...
          form.label("preferred_#{key}", Colibri.t(key) + ": ") +
      ...
      def link_to_add_fields(name, target, options = {})
        ...
        css_classes = options[:class] ? options[:class] + " colibri_add_fields" : "colibri_add_fields"
      ...
      def link_to_remove_fields(name, f, options = {})
        ...
        link_to_with_icon('icon-trash', name, url, :class => "colibri_remove_fields #{options[:class]}", :data => {:action => 'remove'}, :title => Colibri.t(:remove)) + f.hidden_field(:_destroy)
      end

      def colibri_dom_id(record)
        dom_id(record, 'colibri')
      end
