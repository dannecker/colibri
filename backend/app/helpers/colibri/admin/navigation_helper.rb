  ...
  module Admin
    module NavigationHelper
      ...
      def tab(*args)
        ...
        destination_url = options[:url] || colibri.send("#{options[:route]}_path")
        titleized_label = Colibri.t(options[:label], :default => options[:label], :scope => [:admin, :tab]).titleize
      ...
      #
      # Example :
      # :products returns Colibri::Product
      ...
      def klass_for(name)
        ...
        ["Colibri::#{model_name.classify}", model_name.classify, model_name.gsub('_', '/').classify].find do |t|
          t.safe_constantize
      ...
      def link_to_clone(resource, options={})
        ...
        link_to_with_icon('icon-copy', Colibri.t(:clone), clone_object_url(resource), options)
      end

      def link_to_new(resource)
        options[:data] = {:action => 'new'}
        link_to_with_icon('icon-plus', Colibri.t(:new), edit_object_url(resource))
      end

      def link_to_edit(resource, options={})
        url = options[:url] || edit_object_url(resource)
        options[:data] = {:action => 'edit'}
        link_to_with_icon('icon-edit', Colibri.t(:edit), url, options)
      end
      ...
      def link_to_delete(resource, options={})
        ...
        name = options[:name] || Colibri.t(:delete)
        options[:class] = "delete-resource"
        options[:data] = { :confirm => Colibri.t(:are_you_sure), :action => 'remove' }
