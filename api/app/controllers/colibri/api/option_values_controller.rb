module Colibri
  module Api
    class OptionValuesController < Colibri::Api::BaseController
      def index
        if params[:ids]
      ...
      def create
        authorize! :create, Colibri::OptionValue
      ...
      private
        ...
        def scope
          if params[:option_type_id]
            @scope ||= Colibri::OptionType.find(params[:option_type_id]).option_values.accessible_by(current_ability, :read)
          else
            @scope ||= Colibri::OptionValue.accessible_by(current_ability, :read).load
          end
        end
