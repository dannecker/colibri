...
module Colibri
  describe Api::OrdersController do
    ...
    let(:current_api_user) do
      user = Colibri.user_class.new(:email => "colibri@example.com")
    ...
    it "can not view someone else's order" do
      Order.any_instance.stub :user => stub_model(Colibri::LegacyUser)
      api_get :show, :id => order.to_param
      assert_unauthorized!
    end
    ...
    it "can view an order if the token is passed in header" do
      request.headers["X-Colibri-Order-Token"] = order.token
      api_get :show, :id => order.to_param
      response.status.should == 200
    end

    context "with BarAbility registered" do
      before { Colibri::Ability.register_ability(::BarAbility) }
      after { Colibri::Ability.remove_ability(::BarAbility) }

      it "can view an order" do
        user = mock_model(Colibri::LegacyUser)
        user.stub(:has_colibri_role?).with('bar').and_return(true)
        user.stub(:has_colibri_role?).with('admin').and_return(false)
    ...
    it "can specify additional parameters for a line item" do
      Order.should_receive(:create!).and_return(order = Colibri::Order.new)
    ...
    context "admin user imports order" do
      before { current_api_user.stub has_colibri_role?: true }
      ...
      context "line_items hash not present in request" do
        it "responds successfully" do
          api_put :update, :id => order.to_param, :order => {
            :email => "hublock@colibricommerce.com"
          }
    ...
    context "as an admin" do
      ...
      context "with no orders" do
        before { Colibri::Order.delete_all }
      ...
      context "search" do
        before do
          create(:order)
          Colibri::Order.last.update_attribute(:email, 'colibri@colibricommerce.com')
        end

        let(:expected_result) { Colibri::Order.last }

        it "can query the results through a parameter" do
          api_get :index, :q => { :email_cont => 'colibri' }
          json_response["orders"].count.should == 1
          json_response["orders"].first.should have_attributes(attributes)
      ...
      context "can cancel an order" do
        before do
          Colibri::Config[:mails_from] = "colibri@example.com"
