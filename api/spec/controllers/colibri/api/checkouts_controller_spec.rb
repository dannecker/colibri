  ...
  describe Api::CheckoutsController do
    ...
    context "POST 'create'" do
      ...
      it "assigns email when creating a new order" do
        api_post :create, :order => { :email => "guest@colibricommerce.com" }
        expect(json_response['email']).not_to eq controller.current_api_user
        expect(json_response['email']).to eq "guest@colibricommerce.com"
      end
    end
    ...
    context "PUT 'update'" do
      ...
      it "returns errors when source is missing attributes" do
        ...
        api_put :update, :id => order.to_param, :order_token => order.token,
          ...
          :payment_source => {
            @payment_method.id.to_s => { name: "Colibri" }
          }
      ...
      it "can transition from confirm to complete" do
        ...
        Colibri::Order.any_instance.stub(:payment_required? => false)
        api_put :update, :id => order.to_param, :order_token => order.token
      ...
      context "as an admin" do
        ...
        it "can assign a user to the order" do
          ...
          api_put :update, :id => order.to_param, :order_token => order.token,
            :order => { :user_id => user.id, :email => "guest@colibricommerce.com" }
          response.status.should == 200
      ...
      it "can assign an email to the order" do
        api_put :update, :id => order.to_param, :order_token => order.token,
          :order => { :email => "guest@colibricommerce.com" }
        json_response['email'].should == "guest@colibricommerce.com"
        response.status.should == 200
      end
    ...
    context "PUT 'next'" do
      ...
      it "cannot transition to address without a line item" do
        ...
        response.status.should == 422
        json_response["errors"]["base"].should include(Colibri.t(:there_are_no_items_for_this_order))
      end
      ...
      it "returns a sensible error when no payment method is specified" do
        ...
        json_response["errors"]["base"].should include(Colibri.t(:no_pending_payments))
      end
    end
    ...
    context "PUT 'advance'" do
      ...
      it 'continues to advance advances an order while it can move forward' do
        Colibri::Order.any_instance.should_receive(:next).exactly(3).times.and_return(true, true, false)
