...
module Colibri
  describe Api::ZonesController do
    ...
    context "specifying a rabl template to use" do
      before do
        Colibri::Api::ZonesController.class_eval do
          def custom_show
            respond_with(zone)
          end
        end
      ...
      it "uses the specified template" do
        request.headers['X-Colibri-Template'] = 'show'
        api_get :custom_show, :id => @zone.id
        response.should render_template('colibri/api/zones/show')
      end

      it "falls back to the default template if the specified template does not exist" do
        request.headers['X-Colibri-Template'] = 'invoice'
        api_get :show, :id => @zone.id
        response.should render_template('colibri/api/zones/show')
      end
    end
